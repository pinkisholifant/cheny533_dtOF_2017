...
//    void ofImage::setImageType(ofImageType type);
//    ofImage<myImage>(<#const string &filename#>);
//    myImage.loadImage("somethingColor.jpg");
    ...
    flowField = FlowField(ofGetWidth(), ofGetHeight(), 10.);
...
void ofApp::keyReleased(int key){
	...
	if (key == ' ')
	{
		...
		for (int i=0; i<particles.size(); i++)
		{
			particles[i].pos.x = ofRandom(0, 100);
            particles[i].pos.x = ofRandom(300, 362);
            //particles[i].pos.x = 0;
